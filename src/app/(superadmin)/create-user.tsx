...
export default function CreateUserScreen() {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      if (!userProfile.permissions?.canManageAdmins) {
      ...
      console.log('[SUPERADMIN] Creating user:', email.toLowerCase());
      ...
      await firestore()
        ...
        .set({
          ...
          permissions: role === 'superadmin' 
            ? { canManageAdmins: true, canManageUsers: true, canViewAuditLogs: true, canModifySettings: true }
            : role === 'admin'
      ...
      console.log('[SUPERADMIN] Created new user:', newUserUid);

      if (adminUid && adminEmail) {
        const currentSessionUid = auth().currentUser?.uid;
        if (currentSessionUid !== adminUid) {
          await auth().signOut();
          console.log('[SUPERADMIN] Switched auth back to admin');
        }
      }
        ...
        <View className="space-y-6">
          ...
          <View>
            ...
            <View className="space-y-2">
              ...
              >
                <MaterialCommunityIcons 
                  name={role === 'superadmin' ? 'radiobox-marked' : 'radiobox-blank'} 
                  size={24} 
                  color={role === 'superadmin' ? '#ef4444' : '#9ca3af'} 
                />
                <View className="ml-3 flex-1">
                  <AppText className="text-base color-white font-semibold">Super Administrator</AppText>
                  <AppText className="text-sm color-neutral-400">Full system access - all permissions</AppText>
                </View>
              </TouchableOpacity>
            </View>
          </View>
